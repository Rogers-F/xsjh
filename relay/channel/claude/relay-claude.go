...
// claudeTextFileExtensions is the outbound policy allowlist of file extensions whose
// base64 payload is decoded into a plain text block (NOT the generic MIME map in
// service.GetMimeTypeByExtension, which classifies html as text and lacks log/yaml).
var claudeTextFileExtensions = map[string]bool{
	".txt": true, ".md": true, ".markdown": true, ".csv": true, ".log": true,
	".json": true, ".xml": true, ".yaml": true, ".yml": true,
}

// claudeImageFileExtensions routes image files attached via the chat-completions
// `file` content part to the legacy MIME-sniffing image path (compatibility with
// clients that send images as files).
var claudeImageFileExtensions = map[string]bool{
	".jpg": true, ".jpeg": true, ".png": true, ".gif": true, ".webp": true,
	".jfif": true, ".heic": true, ".heif": true,
}

// stripDataURIPrefix returns the bare base64 payload of a `data:*;base64,...` URI, or
// the input unchanged when no such prefix is present.
...
// convertFileContentToClaude maps a chat-completions `file` content part onto an
// upstream content block by filename extension: .pdf becomes a document block (base64
// passed through verbatim), allowlisted text files are decoded into a text block,
// image extensions delegate to the legacy sniffing path, and everything else
// (file_id-only refs, undecodable text, unsupported extensions) yields nil so the
// caller drops it.
func convertFileContentToClaude(c *gin.Context, mediaMessage dto.MediaContent) (*dto.ClaudeMediaMessage, error) {
	file := mediaMessage.GetFile()
	if file == nil || file.FileData == "" {
		return nil, nil
	}
	ext := strings.ToLower(path.Ext(file.FileName))
	data := stripDataURIPrefix(file.FileData)
	switch {
	case ext == ".pdf":
	...
	case claudeTextFileExtensions[ext]:
	...
	case claudeImageFileExtensions[ext]:
		return convertMediaSourceToClaude(c, mediaMessage)
	default:
		return nil, nil
	}
}

// convertMediaSourceToClaude is the legacy media path: resolve the part's file source,
// fetch/normalize it to base64, and classify document-vs-image by sniffed MIME type.
func convertMediaSourceToClaude(c *gin.Context, mediaMessage dto.MediaContent) (*dto.ClaudeMediaMessage, error) {
	...
	block.Source.MediaType = mimeType
		...
		if message.Role == "system" {
			...
			if message.Role == "tool" {
				...
				for _, mediaMessage := range message.ParseContent() {
					...
					default:
						convert := convertMediaSourceToClaude
						if mediaMessage.Type == dto.ContentTypeFile {
							convert = convertFileContentToClaude
						}
						block, err := convert(c, mediaMessage)
						if err != nil {
							return nil, err
						}
						// nil = unsupported part: dropped rather than mis-sent as an image.
						if block != nil {
