...
// sanitizeOAuthOutboundHeaders is the FINAL defense for the Claude OAuth path. It runs
// after SetupRequestHeader AND after channel header overrides (which have no denylist and
// could otherwise inject Authorization / x-api-key / Host / Cookie). It enforces the
// exact CLI header allowlist, resets Host to the upstream, strips credential/cookie
// leakage, and re-asserts the fresh Bearer so an override cannot replace it. No-op on
// every non-OAuth path.
func sanitizeOAuthOutboundHeaders(req *http.Request, info *common.RelayInfo) {
	if req == nil || info == nil || !info.IsOAuth {
		return
	}
	if req.URL != nil {
		req.Host = req.URL.Host
	}
	allowed := info.OAuthAllowedHeaders
	for name := range req.Header {
		lower := strings.ToLower(name)
		if lower == "authorization" {
			continue // re-asserted below
		}
		if allowed != nil && allowed[lower] {
			continue
		}
		req.Header.Del(name)
	}
	// Re-assert the CLI fingerprint (UA / X-Stainless-* / anthropic-version / anthropic-beta).
	// These names are allowlisted, so a channel header override could otherwise replace them
	// and break the mimic — force them back to the adaptor's authoritative values.
	for k, v := range info.OAuthForcedHeaders {
		req.Header.Set(k, v)
	}
	// Belt-and-suspenders: these must never reach the OAuth upstream.
	req.Header.Del("Cookie")
	req.Header.Del("X-Api-Key")
	// Re-assert the fresh Bearer; a header override must not be able to swap it out.
	if info.OAuthOutboundBearer != "" {
		req.Header.Set("Authorization", info.OAuthOutboundBearer)
	} else {
...
func doRequest(c *gin.Context, req *http.Request, info *common.RelayInfo) (*http.Response, error) {
	sanitizeOAuthOutboundHeaders(req, info)
