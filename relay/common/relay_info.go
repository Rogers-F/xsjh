...
type ChannelMeta struct {
	...
	// IsOAuth marks a Claude(Anthropic) channel that authenticates with an OAuth
	// subscription credential (rotating Bearer) instead of a static x-api-key.
	// Derived once in InitChannelMeta and is the single source of truth read by
	// every OAuth-path branch (header build, body transform, outbound sanitizer).
	IsOAuth bool
	// OAuthOutboundBearer is the full "Bearer <access_token>" value re-asserted by
	// the final outbound sanitizer (defense-in-depth after header overrides run).
	// Set by the Claude adaptor's SetupRequestHeader on the OAuth path; empty otherwise.
	OAuthOutboundBearer string
	// OAuthAllowedHeaders is the lowercase allowlist the final outbound sanitizer
	// enforces; any header not in this set is stripped before the request is sent.
	// Set by the Claude adaptor on the OAuth path; nil (no-op) on every other path.
	OAuthAllowedHeaders map[string]bool
	// OAuthForcedHeaders are the CLI-fingerprint headers (UA, X-Stainless-*, anthropic-
	// version/beta, …) the adaptor force-set. The final sanitizer RE-ASSERTS them after
	// channel header overrides run, so an override cannot tamper with the mimic
	// fingerprint (those header names are allowlisted and would otherwise survive).
	OAuthForcedHeaders map[string]string
}
...
func (info *RelayInfo) InitChannelMeta(c *gin.Context) {
	...
	// Anthropic channels may authenticate via an OAuth subscription credential
	// instead of a static x-api-key. Derive the gate once here so downstream code
	// (header build / body transform / outbound sanitizer) reads a single flag.
	if channelType == constant.ChannelTypeAnthropic && channelMeta.ChannelOtherSettings.IsOAuthMode() {
		channelMeta.IsOAuth = true
	}
