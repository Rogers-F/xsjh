...
// CompareAndSwapChannelKey atomically updates a channel's key only if its current
// stored key still equals oldKey (exact-key optimistic CAS). Returns true when the
// row was updated. The Claude OAuth refresh coordinator uses this so a losing
// concurrent refresh never clobbers the winner's freshly rotated credential, and
// without adding a dedicated version column (the full key blob IS the version).
func CompareAndSwapChannelKey(id int, oldKey, newKey string) (bool, error) {
	res := DB.Model(&Channel{}).Where("id = ? AND "+commonKeyCol+" = ?", id, oldKey).Update("key", newKey)
	if res.Error != nil {
...
// GetChannelsByTypeWithKey loads all channels of a given type INCLUDING their key
// column (GetChannelsByType omits it). Used by the Claude OAuth refresh coordinator
// to fan a rotated token out to sibling channels sharing one subscription.
func GetChannelsByTypeWithKey(channelType int) ([]*Channel, error) {
	var channels []*Channel
