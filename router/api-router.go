...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.SessionCORS())
	{
		...
		apiRouter.GET("/public-settings", controller.GetPublicSettings)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			// POST is the canonical (state-changing) logout; GET is kept for any
			// cached old bundle. Both go through the same handler, which enforces a
			// New-Api-User CSRF guard (a cross-site request can't set that header).
		...
		channelRoute.Use(middleware.AdminAuth())
		{
			...
			channelRoute.POST("/claude/oauth/validate", controller.ValidateClaudeOAuthCredential)
			channelRoute.POST("/:id/claude/refresh", controller.RefreshClaudeOAuthCredential)
