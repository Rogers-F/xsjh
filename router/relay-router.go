...
func SetRelayRouter(router *gin.Engine) {
	// Engine-global, path-aware CORS. It is the only CORS in the gin NoRoute chain,
	// so it answers ALL preflight (OPTIONS) per-surface (session vs relay); a bare
	// relay CORS here would mis-answer the cookie surfaces' (/api, /pg) preflight.
	router.Use(middleware.GlobalCORS())
