...
var (
	...
	// maskBearerPattern masks "Bearer <token>" (OAuth access tokens in headers/logs).
	maskBearerPattern = regexp.MustCompile(`(?i)(bearer\s+)[^\s'"]+`)
	// maskOAuthTokenPattern masks JSON/kv access_token / refresh_token / id_token values.
	maskOAuthTokenPattern = regexp.MustCompile(`(['"]?(?:access_token|refresh_token|id_token)['"]?\s*[:=]\s*['"]?)[^\s'",}]+(['"]?)`)
)
...
func MaskSensitiveInfo(str string) string {
	...
	// Mask OAuth tokens BEFORE the api_key rule: Bearer access tokens and
	// access_token/refresh_token/id_token values must never survive in logs.
	str = maskBearerPattern.ReplaceAllString(str, "${1}***")
	str = maskOAuthTokenPattern.ReplaceAllString(str, "${1}***${2}")
