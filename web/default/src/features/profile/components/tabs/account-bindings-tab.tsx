...
export function AccountBindingsTab({
  ...
  const handleBindCustomOAuth = (provider: { id: string; name: string }) => {
    // The callback page lives under the console basepath; the /api path stays root-absolute.
    const redirectUrl = `${window.location.origin}${APP_BASEPATH}/oauth/${provider.id}?bind=true`
