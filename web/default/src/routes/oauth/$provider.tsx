  ...
  useEffect(() => {
    ;(async () => {
        ...
        if (typeof window !== 'undefined') {
          setTimeout(() => {
            const normalizedTarget = target.startsWith('/')
              ? target
              : `/${target}`
            // The router lives under APP_BASEPATH, so the hard-redirect
            // fallback must compare against (and land on) the prefixed URL.
            const prefixedTarget =
              normalizedTarget === APP_BASEPATH ||
              normalizedTarget.startsWith(`${APP_BASEPATH}/`)
                ? normalizedTarget
                : `${APP_BASEPATH}${normalizedTarget}`
            const currentPath =
              window.location.pathname + window.location.search
            if (
              currentPath !== prefixedTarget &&
              currentPath !== `${prefixedTarget}/`
            ) {
              window.location.replace(prefixedTarget)
            }
          }, 100)
        }
      }
      ...
      const closeBindingWindow = () => {
        ...
        setTimeout(() => {
          if (!window.closed) {
            window.location.replace(`${APP_BASEPATH}/_authenticated/profile/`)
          }
        }, 200)
      }
