...
export async function onDiscordOAuthClicked(client_id, options = {}) {
  ...
  if (!state) return;
  const redirect_uri = `${window.location.origin}${APP_BASEPATH}/oauth/discord`;
...
export async function onOIDCClicked(
  ...
  url.searchParams.set(
    'redirect_uri',
    `${window.location.origin}${APP_BASEPATH}/oauth/oidc`,
  );
...
export async function onCustomOAuthClicked(provider, options = {}) {
  ...
  try {
    const redirect_uri = `${window.location.origin}${APP_BASEPATH}/oauth/${provider.slug}`;
