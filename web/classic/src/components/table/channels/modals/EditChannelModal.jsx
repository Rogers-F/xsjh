...
import ClaudeOAuthModal from './ClaudeOAuthModal';
...
const EditChannelModal = (props) => {
  ...
  const [codexCredentialRefreshing, setCodexCredentialRefreshing] =
    useState(false);
  const [claudeOAuthModalVisible, setClaudeOAuthModalVisible] = useState(false);
  const [claudeCredentialRefreshing, setClaudeCredentialRefreshing] =
    useState(false);
    ...
    if (success) {
      ...
      if (data.settings) {
        try {
          ...
          data.auth_mode = parsedSettings.auth_mode || '';
  ...
  const handleClaudeOAuthGenerated = (key) => {
    handleInputChange('key', key);
    handleChannelOtherSettingsChange('auth_mode', 'oauth');
    formatJsonField('key');
  };

  const handleRefreshClaudeCredential = async () => {
    if (!isEdit) return;

    setClaudeCredentialRefreshing(true);
    try {
      const res = await API.post(
      ...
      setClaudeCredentialRefreshing(false);
    }
  };
  ...
  const submit = async () => {
    ...
    if (localInputs.type === 1 || localInputs.type === 14) {
      ...
      if (localInputs.type === 14) {
        ...
        settings.auth_mode = localInputs.auth_mode === 'oauth' ? 'oauth' : '';
      }
    }
                  ...
                  {inputs.type === 14 && (
                    <>
                      <Form.Select
                        field='auth_mode'
                        label={t('认证模式')}
                        onChange={(value) => handleChannelOtherSettingsChange('auth_mode', value)}
                        optionList={[
                          { label: t('API Key（默认）'), value: '' },
                          { label: t('OAuth 订阅'), value: 'oauth' },
                        ]}
                        style={{ width: '100%' }}
                        extraText={t('OAuth 订阅模式下，密钥需为 Claude.ai 订阅凭据 JSON；可用下方按钮粘贴 refresh_token 自动校验生成')}
                      />
                      <div className='flex gap-2 mt-2'>
                        <Button
                          theme='outline'
                          type='primary'
                          onClick={() => setClaudeOAuthModalVisible(true)}
                        >
                          {t('导入 OAuth 订阅')}
                        </Button>
                        {isEdit && inputs.auth_mode === 'oauth' && (
                          <Button
                            theme='outline'
                            onClick={handleRefreshClaudeCredential}
                            loading={claudeCredentialRefreshing}
                          >
                            {t('刷新凭证')}
                          </Button>
                        )}
                      ...
                      <ClaudeOAuthModal
                        visible={claudeOAuthModalVisible}
                        onCancel={() => setClaudeOAuthModalVisible(false)}
                        onSuccess={handleClaudeOAuthGenerated}
                        defaultProxy={inputs.proxy}
                      />
                    </>
                  )}
