<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-paper-50 px-4 py-2.5 text-sm dark:bg-ink-700">
      ...
      <div class="rounded-lg border border-paper-200 p-3 dark:border-ink-700">
        <!-- 添加用户 -->
        <h4 class="mb-2 text-sm font-medium text-dust-700 dark:text-pearl-100">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-y-auto rounded-lg border border-paper-200 bg-white shadow-lg dark:border-ink-600 dark:bg-ink-700"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-paper-50 dark:hover:bg-ink-700"
        ...
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-paper-200 pt-3 dark:border-ink-700">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded border border-paper-200 bg-white px-2 py-1 text-center text-sm transition-colors focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500/20 dark:border-ink-600 dark:bg-ink-700 dark:focus:border-gold-500"
              placeholder="0.5"
            />
            <button
              type="button"
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="rounded-lg border border-coral-500/30 bg-coral-500/10 px-3 py-1.5 text-sm font-medium text-coral-600 transition-colors hover:bg-coral-500/15 dark:border-coral-500/30 dark:bg-coral-500/10 dark:text-coral-500 dark:hover:bg-coral-500/15"
              @click="clearAllLocal"
            >
      ...
      <div v-if="loading" class="flex justify-center py-6">
        <svg class="h-6 w-6 animate-spin text-gold-500" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else>
        ...
        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-dust-400 dark:text-pearl-400">
          {{ t('admin.groups.noRateMultipliers') }}
        </div>

        <div v-else>
          <!-- 表格 -->
          <div class="overflow-hidden rounded-lg border hairline-strong">
              ...
              <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-paper-200 dark:divide-ink-700">
                  <tr
                    ...
                    :key="entry.user_id"
                    class="hover:bg-paper-50 dark:hover:bg-white/[0.04]/50"
                  >
                      ...
                      <span
                        :class="[
                          ...
                          entry.user_status === 'active'
                            ...
                            : 'bg-paper-100 text-dust-600 dark:bg-ink-700 dark:text-pearl-300'
                        ]"
                      >
                        {{ entry.user_status }}
                      </span>
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        class="hide-spinner w-20 rounded border border-paper-200 bg-white px-2 py-1 text-center text-sm font-medium transition-colors focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500/20 dark:border-ink-600 dark:bg-ink-700 dark:focus:border-gold-500"
                        @change="updateLocalRate(entry.user_id, ($event.target as HTMLInputElement).value)"
                      />
                    </td>
                    <td v-if="showFinalRate" class="whitespace-nowrap px-3 py-2 font-medium text-gold-600 dark:text-gold-300">
                      {{ computeFinalRate(entry.rate_multiplier) }}
                    </td>
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-dust-400 transition-colors hover:bg-coral-500/10 hover:text-coral-700 dark:hover:bg-coral-500/15 dark:hover:text-coral-500"
      ...
      <!-- 底部操作栏 -->
      <div class="flex items-center gap-3 border-t border-paper-200 pt-4 dark:border-ink-700">
        <!-- 左侧：未保存提示 + 撤销 -->
        <template v-if="isDirty">
          <span class="text-xs text-amber-600 dark:text-amber-400">{{ t('admin.groups.unsavedChanges') }}</span>
          <button
            type="button"
            class="text-xs font-medium text-gold-600 hover:text-gold-700 dark:text-gold-300 dark:hover:text-gold-200"
            @click="handleCancel"
          >
...
const platformColorClass = computed(() => {
  switch (props.group?.platform) {
    ...
    case 'openai': return 'text-mint-700 dark:text-mint-400'
