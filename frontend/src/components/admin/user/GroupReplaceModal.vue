<template>
  <BaseDialog :show="show" :title="t('admin.users.replaceGroupTitle')" width="narrow" @close="$emit('close')">
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ? 'border-gold-500/80 bg-gold-500/10 dark:border-gold-300 dark:bg-gold-300/10'
            : 'border-paper-200 hover:hairline-strong dark:hover:border-ink-600'"
        >
          <input
            type="radio"
            :value="group.id"
      ...
      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-dust-400">
