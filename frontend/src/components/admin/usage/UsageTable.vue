<template>
      ...
      <DataTable :columns="columns" :data="data" :loading="loading">
        <template #cell-user="{ row }">
          <div class="text-sm">
            <button
              v-if="row.user?.email"
              class="font-medium text-gold-600 underline decoration-dashed underline-offset-2 transition-colors hover:text-gold-700 dark:text-gold-300 dark:hover:text-gold-200"
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-else class="flex items-center gap-1.5">
            ...
            >
              <div class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-paper-100 transition-colors group-hover:bg-blue-100 dark:bg-ink-700 dark:group-hover:bg-blue-900/50">
                <Icon name="infoCircle" size="xs" class="text-dust-400 group-hover:text-blue-500 dark:text-pearl-500 dark:group-hover:text-blue-400" />
              </div>
            </div>
          </div>
        </template>
        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            ...
            <div v-if="row.account_rate_multiplier != null" class="mt-0.5 text-[11px] text-dust-400">
        ...
        <template #cell-first_token="{ row }">
          <span v-if="row.first_token_ms != null" class="text-sm text-dust-600 dark:text-pearl-300">{{ formatDuration(row.first_token_ms) }}</span>
          <span v-else class="text-sm text-dust-400 dark:text-pearl-400">-</span>
        </template>

        <template #cell-duration="{ row }">
          <span class="text-sm text-dust-600 dark:text-pearl-300">{{ formatDuration(row.duration_ms) }}</span>
        </template>

        <template #cell-created_at="{ value }">
          <span class="text-sm text-dust-600 dark:text-pearl-300">{{ formatDateTime(value) }}</span>
        </template>

        <template #cell-user_agent="{ row }">
          <span v-if="row.user_agent" class="text-sm text-dust-600 dark:text-pearl-300 block max-w-[320px] truncate" :title="row.user_agent">{{ formatUserAgent(row.user_agent) }}</span>
          <span v-else class="text-sm text-dust-400 dark:text-pearl-400">-</span>
        </template>

        <template #cell-ip_address="{ row }">
          <span v-if="row.ip_address" class="text-sm font-mono text-dust-600 dark:text-pearl-300">{{ row.ip_address }}</span>
          <span v-else class="text-sm text-dust-400 dark:text-pearl-400">-</span>
        </template>
  ...
  <Teleport to="body">
    ...
    >
      <div class="whitespace-nowrap rounded-lg border border-ink-700 bg-ink-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-ink-700 dark:bg-ink-800">
        <div class="space-y-1.5">
          <div>
            <div class="text-xs font-semibold text-dust-300 mb-1">{{ t('usage.tokenDetails') }}</div>
            <div v-if="tokenTooltipData && tokenTooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('admin.usage.inputTokens') }}</span>
              <span class="font-medium text-white">{{ tokenTooltipData.input_tokens.toLocaleString() }}</span>
            </div>
            <div v-if="tokenTooltipData && tokenTooltipData.output_tokens > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('admin.usage.outputTokens') }}</span>
            ...
            <div v-if="tokenTooltipData && tokenTooltipData.cache_creation_tokens > 0">
              ...
              <template v-if="tokenTooltipData.cache_creation_5m_tokens > 0 || tokenTooltipData.cache_creation_1h_tokens > 0">
                <div v-if="tokenTooltipData.cache_creation_5m_tokens > 0" class="flex items-center justify-between gap-4">
                  <span class="text-dust-400 flex items-center gap-1.5">
              ...
              <div v-else class="flex items-center justify-between gap-4">
                <span class="text-dust-400">{{ t('admin.usage.cacheCreationTokens') }}</span>
            ...
            <div v-if="tokenTooltipData && tokenTooltipData.cache_read_tokens > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('admin.usage.cacheReadTokens') }}</span>
              <span class="font-medium text-white">{{ tokenTooltipData.cache_read_tokens.toLocaleString() }}</span>
            </div>
          </div>
          <div class="flex items-center justify-between gap-6 border-t border-ink-700 pt-1.5">
            <span class="text-dust-400">{{ t('usage.totalTokens') }}</span>
            <span class="font-semibold text-blue-400">{{ ((tokenTooltipData?.input_tokens || 0) + (tokenTooltipData?.output_tokens || 0) + (tokenTooltipData?.cache_creation_tokens || 0) + (tokenTooltipData?.cache_read_tokens || 0)).toLocaleString() }}</span>
          </div>
        </div>
        <div class="absolute right-full top-1/2 h-0 w-0 -translate-y-1/2 border-b-[6px] border-r-[6px] border-t-[6px] border-b-transparent border-r-ink-900 border-t-transparent dark:border-r-ink-800"></div>
      </div>
    </div>
  </Teleport>
      ...
      <div class="whitespace-nowrap rounded-lg border border-ink-700 bg-ink-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-ink-700 dark:bg-ink-800">
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-ink-700 pb-1.5">
            <div class="text-xs font-semibold text-dust-300 mb-1">{{ t('usage.costDetails') }}</div>
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('admin.usage.inputCost') }}</span>
              <span class="font-medium text-white">${{ tooltipData.input_cost.toFixed(6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.output_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('admin.usage.outputCost') }}</span>
              <span class="font-medium text-white">${{ tooltipData.output_cost.toFixed(6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('usage.inputTokenPrice') }}</span>
              <span class="font-medium text-sky-300">{{ formatTokenPricePerMillion(tooltipData.input_cost, tooltipData.input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.output_tokens > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('usage.outputTokenPrice') }}</span>
              <span class="font-medium text-violet-300">{{ formatTokenPricePerMillion(tooltipData.output_cost, tooltipData.output_tokens) }} {{ t('usage.perMillionTokens') }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('admin.usage.cacheCreationCost') }}</span>
              <span class="font-medium text-white">${{ tooltipData.cache_creation_cost.toFixed(6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-dust-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">${{ tooltipData.cache_read_cost.toFixed(6) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          <div class="flex items-center justify-between gap-6">
            <span class="text-dust-400">{{ t('usage.serviceTier') }}</span>
            <span class="font-semibold text-cyan-300">{{ getUsageServiceTierLabel(tooltipData?.service_tier, t) }}</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-dust-400">{{ t('usage.rate') }}</span>
            <span class="font-semibold text-blue-400">{{ (tooltipData?.rate_multiplier || 1).toFixed(2) }}x</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-dust-400">{{ t('usage.accountMultiplier') }}</span>
            <span class="font-semibold text-blue-400">{{ (tooltipData?.account_rate_multiplier ?? 1).toFixed(2) }}x</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-dust-400">{{ t('usage.original') }}</span>
            <span class="font-medium text-white">${{ tooltipData?.total_cost?.toFixed(6) || '0.000000' }}</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-dust-400">{{ t('usage.userBilled') }}</span>
            <span class="font-semibold text-green-400">${{ tooltipData?.actual_cost?.toFixed(6) || '0.000000' }}</span>
          </div>
          <div class="flex items-center justify-between gap-6 border-t border-ink-700 pt-1.5">
            <span class="text-dust-400">{{ t('usage.accountBilled') }}</span>
...
const getRequestTypeBadgeClass = (row: AdminUsageLog): string => {
  ...
  if (requestType === 'sync') return 'bg-paper-100 text-dust-800 dark:bg-ink-700 dark:text-pearl-100'
