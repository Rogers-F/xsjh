<template>
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-dust-500">{{ t('usage.totalRequests') }}</p>
        <p class="text-xl font-bold">{{ stats?.total_requests?.toLocaleString() || '0' }}</p>
        <p class="text-xs text-dust-400">{{ t('usage.inSelectedRange') }}</p>
      </div>
    </div>
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-dust-500">{{ t('usage.totalTokens') }}</p>
        <p class="text-xl font-bold">{{ formatTokens(stats?.total_tokens || 0) }}</p>
        <p class="text-xs text-dust-500">
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-xs font-medium text-dust-500">{{ t('usage.totalCost') }}</p>
        <p class="text-xl font-bold text-green-600">
          ${{ ((stats?.total_account_cost ?? stats?.total_actual_cost) || 0).toFixed(4) }}
        </p>
        <p class="text-xs text-dust-400" v-if="stats?.total_account_cost != null">
          {{ t('usage.userBilled') }}:
          <span class="text-dust-300">${{ (stats?.total_actual_cost || 0).toFixed(4) }}</span>
          · {{ t('usage.standardCost') }}:
          <span class="text-dust-300">${{ (stats?.total_cost || 0).toFixed(4) }}</span>
        </p>
        <p class="text-xs text-dust-400" v-else>
          {{ t('usage.standardCost') }}:
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div><p class="text-xs font-medium text-dust-500">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>
