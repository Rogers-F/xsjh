<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        v-if="showAddForm"
        class="rounded-xl border border-gold-500/30 bg-gold-500/10 p-4 dark:border-gold-300/40 dark:bg-gold-300/10"
      >
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-dust-600 dark:text-pearl-300">
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-dust-600 dark:text-pearl-300">
              {{ t('admin.scheduledTests.cronExpression') }}
              <HelpTooltip>
                <template #trigger>
                  <span class="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-paper-300/70 text-[10px] font-semibold text-dust-400 transition-colors hover:border-gold-500 hover:text-gold-700 dark:border-pearl-400/30 dark:text-pearl-500 dark:hover:border-gold-300 dark:hover:text-gold-200">
                    ?
                  </span>
                </template>
          ...
          <div class="flex items-end">
            <label class="flex items-center gap-2 text-sm text-dust-700 dark:text-pearl-100">
          ...
          <div class="flex items-end">
            <div>
              ...
              <p class="mt-0.5 text-xs text-dust-400 dark:text-pearl-400">
        ...
        <div class="mt-3 flex justify-end gap-2">
          <button
            ...
            class="rounded-lg bg-paper-100 px-3 py-1.5 text-sm font-medium text-dust-700 transition-colors hover:bg-paper-200 dark:bg-ink-700 dark:text-pearl-200 dark:hover:bg-ink-600"
          >
            {{ t('common.cancel') }}
          </button>
          <button
            ...
            class="flex items-center gap-1.5 rounded-lg bg-gold-500 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-gold-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-dust-400" :stroke-width="2" />
        <span class="ml-2 text-sm text-dust-500">{{ t('common.loading') }}...</span>
      </div>
      ...
      <div v-else class="space-y-3">
        <div
          v-for="plan in plans"
          :key="plan.id"
          class="rounded-xl border border-paper-200 bg-white transition-all dark:border-ink-700 dark:bg-ink-800"
        >
          <!-- Plan Header -->
          <div
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              ...
              <span
                ...
                class="inline-flex items-center rounded-full bg-mint-500/15 px-2 py-0.5 text-xs font-medium text-mint-700 dark:bg-mint-500/20 dark:text-mint-400"
              >
                {{ t('admin.scheduledTests.autoRecover') }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex items-center gap-1" @click.stop>
                <button
                  ...
                  class="rounded-lg p-1.5 text-dust-400 transition-colors hover:bg-blue-50 hover:text-blue-500 dark:hover:bg-blue-900/20"
                ...
                <button
                  ...
                  class="rounded-lg p-1.5 text-dust-400 transition-colors hover:bg-coral-500/10 hover:text-coral-600 dark:text-coral-500 dark:hover:bg-coral-500/15"
              ...
              <Icon
                ...
                size="sm"
                :class="[
                  'text-dust-400 transition-transform duration-200',
          ...
          <div
            ...
            @click.stop
          >
            <div class="mb-2 text-xs font-medium text-dust-600 dark:text-pearl-300">
          ...
          <div
            ...
            class="border-t border-paper-200 px-4 py-3 dark:border-ink-700"
          >
            <div class="mb-2 text-xs font-medium text-dust-600 dark:text-pearl-300">
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-dust-400" :stroke-width="2" />
              <span class="ml-2 text-xs text-dust-500">{{ t('common.loading') }}...</span>
            </div>
            ...
            <div v-else class="max-h-64 space-y-2 overflow-y-auto">
              <div
                ...
                :key="result.id"
                class="rounded-lg border border-paper-200 bg-paper-50 p-3 dark:border-ink-700 dark:bg-ink-900"
              >
                <div class="flex items-center justify-between">
                      ...
                      :class="[
                        ...
                        result.status === 'success'
                          ...
                          : result.status === 'running'
                            ...
                            : 'bg-coral-500/15 text-coral-700 dark:bg-coral-500/20 dark:text-coral-500'
                      ]"
                    >
                      {{
                        result.status === 'success'
                  ...
                  <!-- Started At -->
                  <span class="text-xs text-dust-400">
                ...
                <div v-if="result.error_message" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-coral-600 dark:text-coral-500"
                  ...
                  <pre
                    ...
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-coral-500/10 p-2 text-xs text-coral-700 dark:bg-coral-500/10 dark:text-coral-400"
                  >{{ result.error_message }}</pre>
                </div>
                <div v-else-if="result.response_text" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-dust-600 dark:text-pearl-300"
                    @click="toggleResultDetail(result.id)"
                  >
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-paper-100 p-2 text-xs text-dust-700 dark:bg-ink-800 dark:text-pearl-200"
