<template>
  ...
  >
    ...
    >
      ...
      <div
        class="rounded-lg bg-paper-100 p-3 text-xs text-dust-500 dark:bg-ink-700/60 dark:text-pearl-300"
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-dust-700 dark:text-pearl-100">
          <input
            ...
            type="checkbox"
            class="rounded border-paper-300 dark:border-ink-600"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      <div
        ...
        class="rounded-lg bg-paper-100 p-3 dark:bg-ink-700/60"
      >
        <div class="mb-2 text-sm font-medium text-dust-700 dark:text-pearl-100">
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          ...
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="text-xs text-dust-500 hover:text-dust-700 dark:text-pearl-300"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-lg border border-paper-200 p-2 dark:border-ink-600"
        >
          <label
          ...
          >
            ...
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-dust-700 dark:text-pearl-100">{{ acc.name }}</span>
          </label>
        </div>
        <div class="mt-1 text-xs text-dust-400">
      ...
      <div
        ...
        class="rounded-lg bg-paper-100 p-4 text-center text-sm text-dust-500 dark:bg-ink-700/60 dark:text-pearl-300"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      ...
      >
        ...
        <div v-if="errorItems.length" class="mt-2">
          <div class="text-sm font-medium text-coral-600 dark:text-coral-500">
            {{ t('admin.accounts.syncErrors') }}
          </div>
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-paper-100 p-3 font-mono text-xs dark:bg-ink-800"
          >
