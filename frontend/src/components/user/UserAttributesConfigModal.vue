  ...
  <BaseDialog :show="show" :title="t('admin.users.attributes.title')" width="wide" @close="emit('close')">
    <div class="space-y-4">
      ...
      <div v-else-if="attributes.length === 0" class="py-12 text-center">
        <svg class="mx-auto h-12 w-12 text-dust-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1">
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border hairline bg-white p-3 dark:bg-ink-800"
        >
          <!-- Drag Handle -->
          <div class="cursor-move text-dust-400 hover:text-dust-600 dark:hover:text-pearl-100" :title="t('admin.users.attributes.dragToReorder')">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-dust-900 dark:text-white">{{ attr.name }}</span>
              <span class="rounded bg-paper-100 px-1.5 py-0.5 font-mono text-xs text-dust-500 dark:bg-ink-700 dark:text-pearl-300">
                {{ attr.key }}
              </span>
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-dust-500 hover:bg-paper-100 hover:text-gold-600 dark:hover:bg-white/[0.04] dark:hover:text-gold-400"
              :title="t('common.edit')"
            >
            ...
            <button
              ...
              class="rounded-lg p-1.5 text-dust-500 hover:bg-coral-500/10 hover:text-coral-600 dark:hover:bg-coral-500/15 dark:hover:text-coral-500"
    ...
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div v-if="form.type === 'select' || form.type === 'multi_select'" class="space-y-2">
        ...
        <div v-for="(option, index) in form.options" :key="getOptionKey(option)" class="flex items-center gap-2">
          ...
          <button
            ...
            class="rounded-lg p-1.5 text-dust-500 hover:bg-coral-500/10 hover:text-coral-600"
          >
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          <input v-model="form.required" type="checkbox" class="h-4 w-4 rounded border-paper-300 text-gold-600" />
          <span class="text-sm text-dust-700 dark:text-pearl-100">{{ t('admin.users.attributes.required') }}</span>
        </label>
        <label class="flex items-center gap-2">
          <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-paper-300 text-gold-600" />
          <span class="text-sm text-dust-700 dark:text-pearl-100">{{ t('admin.users.attributes.enabled') }}</span>
        </label>
      </div>
    </form>
