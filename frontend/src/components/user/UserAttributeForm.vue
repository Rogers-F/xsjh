<template>
  <div v-if="attributes.length > 0" class="space-y-4">
    <div v-for="attr in attributes" :key="attr.id">
      <label class="input-label">
        {{ attr.name }}
        <span v-if="attr.required" class="text-coral-500">*</span>
      </label>
      ...
      <div v-else-if="attr.type === 'multi_select'" class="space-y-2">
        ...
        >
          <input
            ...
            class="h-4 w-4 rounded border-paper-300 text-gold-600"
          />
          <span class="text-sm text-dust-700 dark:text-pearl-100">{{ opt.label }}</span>
        </label>
      </div>
  ...
  <div v-else-if="loading" class="flex justify-center py-4">
    <svg class="h-5 w-5 animate-spin text-dust-400" fill="none" viewBox="0 0 24 24">
