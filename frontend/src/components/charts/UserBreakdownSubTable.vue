<template>
  <div class="bg-paper-100/50 dark:bg-white/[0.02]">
    <div v-if="loading" class="flex items-center justify-center py-3">
      <LoadingSpinner />
    </div>
    <div v-else-if="items.length === 0" class="py-2 text-center text-xs text-dust-400 dark:text-pearl-400">
    ...
    <table v-else class="w-full text-xs">
      <tbody>
        <tr
          v-for="user in items"
          :key="user.user_id"
          class="border-t border-paper-200/50 dark:border-ink-700/50"
        >
          ...
          <td class="py-1 pr-1 text-right text-dust-400 dark:text-pearl-400">
