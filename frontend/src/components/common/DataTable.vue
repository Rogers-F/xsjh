<template>
  <div class="md:hidden space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-lg border border-paper-200 bg-white p-4 dark:border-ink-700 dark:bg-ink-900">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="h-4 w-20 animate-pulse rounded bg-paper-200 dark:bg-ink-700"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-paper-200 dark:bg-ink-700"></div>
          </div>
          <div v-if="hasActionsColumn" class="border-t border-paper-200 pt-3 dark:border-ink-700">
            <div class="h-8 w-full animate-pulse rounded bg-paper-200 dark:bg-ink-700"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-paper-200 bg-white p-12 text-center dark:border-ink-700 dark:bg-ink-900">
        <slot name="empty">
    ...
    <template v-else>
      <div
        ...
        class="rounded-lg border border-paper-200 bg-white p-4 dark:border-ink-700 dark:bg-ink-900"
      >
        <div class="space-y-3">
          <div
  ...
  >
    <table class="min-w-full divide-y divide-paper-200 dark:divide-ink-700">
      <thead class="table-header bg-paper-50 dark:bg-ink-800">
        <tr>
          <th
          ...
          >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span v-if="column.sortable" class="text-dust-400 dark:text-pearl-300">
                  <svg
      ...
      <tbody class="table-body divide-y divide-paper-200 bg-white dark:divide-ink-700 dark:bg-ink-900">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-for="column in columns" :key="column.key" :class="['whitespace-nowrap py-4', getAdaptivePaddingClass()]">
            <div class="animate-pulse">
              <div class="h-4 w-3/4 rounded bg-paper-200 dark:bg-ink-700"></div>
            </div>
          </td>
        </tr>
