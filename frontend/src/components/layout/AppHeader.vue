<template>
  <header class="glass sticky top-0 z-30 border-b hairline">
    <div class="flex h-16 items-center justify-between px-4 md:px-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <!-- Balance Display(Xingsuan v2:gold 系胶囊 + font-mono 数字) -->
        <div
          v-if="user"
          class="hidden items-center gap-2 rounded-xl border border-gold-500/30 bg-gold-500/10 px-3 py-1.5 dark:border-gold-300/30 dark:bg-gold-300/10 sm:flex"
        >
          <svg
            class="h-4 w-4 text-gold-600 dark:text-gold-300"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="1.5"
          ...
          <span class="font-mono text-sm font-semibold tabular-nums text-gold-600 dark:text-gold-300">
        ...
        <div v-if="user" class="relative" ref="dropdownRef">
          ...
          >
            <div
              class="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-gold-300 to-gold-500 text-sm font-medium text-dust-900 shadow-sm"
            >
              {{ userInitials }}
            </div>
          ...
          <transition name="dropdown">
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div v-if="showOnboardingButton" class="border-t hairline py-1">
              ...
              <div class="border-t hairline py-1">
                <button
                  @click="handleLogout"
                  class="dropdown-item w-full text-coral-600 hover:bg-coral-500/10 dark:text-coral-500 dark:hover:bg-coral-500/10"
                >
                  <svg
                    class="h-4 w-4"
                    fill="none"
