          ...
          <template #cell-rate_limit="{ row }">
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                ...
                <div class="h-1 w-full overflow-hidden rounded-full bg-paper-200 dark:bg-ink-700">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      row.usage_5h >= row.rate_limit_5h ? 'bg-coral-500' :
                ...
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-dust-400 dark:text-pearl-400 tabular-nums">
              ...
              <div v-if="row.rate_limit_1d > 0">
                ...
                <div class="h-1 w-full overflow-hidden rounded-full bg-paper-200 dark:bg-ink-700">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      row.usage_1d >= row.rate_limit_1d ? 'bg-coral-500' :
                ...
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-dust-400 dark:text-pearl-400 tabular-nums">
              ...
              <div v-if="row.rate_limit_7d > 0">
                ...
                <div class="h-1 w-full overflow-hidden rounded-full bg-paper-200 dark:bg-ink-700">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      row.usage_7d >= row.rate_limit_7d ? 'bg-coral-500' :
                ...
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-dust-400 dark:text-pearl-400 tabular-nums">
              ...
              <button
                ...
                class="mt-0.5 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-dust-500 transition-colors hover:bg-paper-100 hover:text-gold-600 dark:hover:bg-white/[0.04] dark:hover:text-gold-300"
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-dust-400">{{ t('keys.selectGroup') }}</span>
            </template>
        ...
        <div v-if="!showEditModal" class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                ...
                formData.use_custom_key ? 'bg-gold-600 dark:bg-gold-400' : 'bg-paper-200 dark:bg-ink-700'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.use_custom_key">
            <input
              ...
              :class="{ 'border-coral-500 dark:border-coral-500': customKeyError }"
            />
            <p v-if="customKeyError" class="mt-1 text-sm text-coral-600 dark:text-coral-500">{{ customKeyError }}</p>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_rate_limit ? 'bg-gold-600 dark:bg-gold-400' : 'bg-paper-200 dark:bg-ink-700'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            ...
            <div>
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_5h > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-paper-100 px-3 py-2 dark:bg-ink-700 text-sm">
                    <span :class="[
                      'font-medium',
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-paper-200 dark:bg-ink-700">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      selectedKey.usage_5h >= selectedKey.rate_limit_5h ? 'bg-coral-500' :
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_1d > 0" class="mt-2">
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-paper-200 dark:bg-ink-700">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      selectedKey.usage_1d >= selectedKey.rate_limit_1d ? 'bg-coral-500' :
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_7d > 0" class="mt-2">
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-paper-200 dark:bg-ink-700">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      selectedKey.usage_7d >= selectedKey.rate_limit_7d ? 'bg-coral-500' :
