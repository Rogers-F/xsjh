<template>
  <div class="flex h-full min-h-0 flex-col bg-white dark:bg-ink-900">
    <!-- Loading State -->
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col">
      <div class="min-h-0 flex-1 overflow-auto border-b hairline">
        <table class="w-full border-separate border-spacing-0">
          <thead class="sticky top-0 z-10 bg-paper-50 dark:bg-ink-800">
            <tr>
              <th class="border-b border-paper-200 px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-wider text-dust-500 dark:border-ink-700 dark:text-pearl-300">
              ...
              <th class="border-b border-paper-200 px-4 py-2.5 text-right text-[11px] font-bold uppercase tracking-wider text-dust-500 dark:border-ink-700 dark:text-pearl-300">
          ...
          <tbody class="divide-y divide-paper-100 dark:divide-ink-700">
            <tr v-if="rows.length === 0">
              <td colspan="10" class="py-12 text-center text-sm text-dust-400 dark:text-pearl-400">
            ...
            <tr
              v-for="log in rows"
              :key="log.id"
              class="group cursor-pointer transition-colors hover:bg-paper-50/80 dark:hover:bg-ink-800/50"
            ...
            >
              ...
              <td class="whitespace-nowrap px-4 py-2">
                <el-tooltip :content="log.request_id || log.client_request_id" placement="top" :show-after="500">
                  <span class="font-mono text-xs font-medium text-dust-900 dark:text-pearl-200">
              ...
              <td class="px-4 py-2">
                <div class="max-w-[160px]">
                  <el-tooltip v-if="log.inbound_endpoint" :content="formatEndpointTooltip(log)" placement="top" :show-after="500">
                    <span class="truncate font-mono text-[11px] text-dust-700 dark:text-dust-300">
              ...
              <!-- Platform -->
              <td class="whitespace-nowrap px-4 py-2">
                <span class="inline-flex items-center rounded bg-paper-100 px-1.5 py-0.5 text-[10px] font-bold uppercase text-dust-600 dark:bg-ink-700 dark:text-dust-300">
                  {{ log.platform || '-' }}
                </span>
              </td>
                ...
                <div class="max-w-[160px]">
                  <template v-if="hasModelMapping(log)">
                    <el-tooltip :content="modelMappingTooltip(log)" placement="top" :show-after="500">
                      <span class="flex items-center gap-1 truncate font-mono text-[11px] text-dust-700 dark:text-dust-300">
                        <span class="truncate">{{ log.requested_model }}</span>
                        <span class="flex-shrink-0 text-dust-400">→</span>
                        <span class="truncate text-gold-600 dark:text-gold-400">{{ log.upstream_model }}</span>
                      </span>
                    </el-tooltip>
                  </template>
                  <template v-else>
                    <span v-if="displayModel(log)" class="truncate font-mono text-[11px] text-dust-700 dark:text-dust-300" :title="displayModel(log)">
                      {{ displayModel(log) }}
                    </span>
                    <span v-else class="text-xs text-dust-400">-</span>
                  </template>
              ...
              <td class="px-4 py-2">
                 <el-tooltip v-if="log.group_id" :content="t('admin.ops.errorLog.id') + ' ' + log.group_id" placement="top" :show-after="500">
                  <span class="max-w-[100px] truncate text-xs font-medium text-dust-900 dark:text-pearl-200">
                    {{ log.group_name || '-' }}
                  </span>
                </el-tooltip>
                <span v-else class="text-xs text-dust-400">-</span>
              ...
              <td class="whitespace-nowrap px-4 py-2">
                <div class="flex items-center gap-1.5">
                  ...
                  <span
                    ...
                    class="rounded bg-paper-100 px-1.5 py-0.5 text-[10px] font-bold text-dust-600 dark:bg-ink-700 dark:text-dust-300"
                  >
              ...
              <td class="px-4 py-2">
                <div class="max-w-[200px]">
                  <p class="truncate text-[11px] font-medium text-dust-600 dark:text-dust-400" :title="log.message">
              ...
              <td class="whitespace-nowrap px-4 py-2 text-right" @click.stop>
                <div class="flex items-center justify-end gap-3">
                  <button type="button" class="text-gold-600 hover:text-gold-700 dark:text-gold-400 text-xs font-bold" @click="emit('openErrorDetail', log.id)">
      ...
      <!-- Pagination -->
      <div class="bg-paper-50/50 dark:bg-ink-800/50">
        <Pagination
          v-if="total > 0"
          :total="total"
          :page="page"
...
function getTypeBadge(log: OpsErrorLog): { label: string; className: string } {
  ...
  if (isUpstreamRow(log)) {
    return { label: t('admin.ops.errorLog.typeUpstream'), className: 'bg-coral-500/10 text-coral-700 ring-coral-600/20 dark:bg-coral-500/15 dark:text-coral-500 dark:ring-coral-500/30' }
  }
  ...
  if (phase === 'internal' && owner === 'platform') {
    return { label: t('admin.ops.errorLog.typeInternal'), className: 'bg-paper-100 text-dust-800 ring-dust-600/20 dark:bg-ink-700 dark:text-pearl-200 dark:ring-ink-500/40' }
  }

    const fallback = phase || owner || t('common.unknown')
    return { label: fallback, className: 'bg-paper-50 text-dust-700 ring-dust-600/10 dark:bg-ink-900 dark:text-dust-300 dark:ring-ink-700' }
}
...
function getStatusClass(code: number): string {
  if (code >= 500) return 'bg-coral-500/10 text-coral-700 ring-coral-600/20 dark:bg-coral-500/15 dark:text-coral-500 dark:ring-coral-500/30'
  ...
  return 'bg-paper-50 text-dust-700 ring-dust-600/20 dark:bg-ink-900/30 dark:text-dust-400 dark:ring-dust-500/30'
}
