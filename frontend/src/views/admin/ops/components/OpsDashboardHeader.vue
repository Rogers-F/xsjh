...
const healthScoreColor = computed(() => {
  if (isSystemIdle.value) return '#9ca3af' // dust-400
...
const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return 'text-dust-400'
  const score = healthScoreValue.value
  if (score == null) return 'text-dust-400'
...
const dbMiddleClass = computed(() => {
  ...
  if (systemMetrics.value?.db_ok === true) return 'text-mint-600 dark:text-mint-500'
  return 'text-primary-fg'
})
...
const redisMiddleClass = computed(() => {
  ...
  if (systemMetrics.value?.redis_ok === true) return 'text-mint-600 dark:text-mint-500'
  return 'text-primary-fg'
})
...
<template>
  <div :class="['flex flex-col gap-4 rounded-3xl bg-white shadow-sm ring-1 ring-ink-900/5 dark:bg-ink-800 dark:ring-ink-700', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-paper-100 pb-4 dark:border-ink-700">
      <div>
        <h1 class="flex items-center gap-2 text-xl font-black text-primary-fg">
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-dust-500 dark:text-dust-400">
          <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
            <span class="relative flex h-2 w-2">
              <span class="relative inline-flex h-2 w-2 rounded-full" :class="props.loading ? 'bg-dust-400' : 'bg-green-500'"></span>
            </span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-paper-200 dark:bg-ink-700 sm:block"></div>
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-paper-100 text-dust-500 transition-colors hover:bg-paper-200 dark:bg-ink-700 dark:text-dust-400 dark:hover:bg-ink-600"
          :disabled="loading"
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-paper-200 dark:bg-ink-700 sm:block"></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-paper-100 px-3 text-xs font-bold text-dust-700 transition-colors hover:bg-paper-200 dark:bg-ink-700 dark:text-dust-300 dark:hover:bg-ink-600"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-paper-100 text-dust-700 transition-colors hover:bg-paper-200 dark:bg-ink-700 dark:text-dust-300 dark:hover:bg-ink-600"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-paper-50 dark:bg-ink-900 lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-white/60 dark:hover:bg-ink-800/60 md:border-r md:border-paper-200 md:pr-6 dark:md:border-ink-700"
          >
            ...
            >
              <div class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5 dark:bg-ink-800 dark:ring-white/10">
                <h4 class="mb-3 border-b border-paper-100 pb-2 text-sm font-bold text-dust-900 dark:border-ink-700 dark:text-pearl-50 flex items-center gap-2">
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    <div class="mt-0.5 shrink-0">
                      <svg v-if="item.type === 'critical'" class="h-4 w-4 text-coral-500" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-primary-fg">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-dust-500 dark:text-dust-400">{{ item.impact }}</div>
                ...
                <div class="mt-3 border-t border-paper-100 pt-2 text-[10px] text-dust-400 dark:border-ink-700">
            ...
            <div class="relative flex items-center justify-center">
              <svg :width="circleSize" :height="circleSize" class="-rotate-90 transform">
                <circle
                  ...
                  fill="transparent"
                  class="text-pearl-200 dark:text-ink-700"
                  stroke="currentColor"
                />
                <circle
                  :cx="circleSize / 2"
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-dust-400']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-dust-500">
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-xs font-bold uppercase tracking-wider text-dust-400">{{ t('admin.ops.realtime.title') }}</h3>
              ...
              <div class="flex flex-wrap gap-1">
                <button
                  ...
                  :class="realtimeWindow === window
                    ...
                    : 'bg-paper-200 text-dust-600 hover:bg-paper-300 dark:bg-ink-700 dark:text-dust-400 dark:hover:bg-ink-600'"
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-dust-400']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-primary-fg']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-dust-500']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-primary-fg']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-dust-500']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-dust-400']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-dust-600 dark:text-dust-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-primary-fg">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-primary-fg">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-dust-400']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-dust-600 dark:text-dust-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-primary-fg">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-primary-fg">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-900" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-dust-400">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-primary-fg">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-primary-fg">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-primary-fg">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-primary-fg">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-900" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-dust-400">{{ t('admin.ops.sla') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.sla')" />
              <span class="h-1.5 w-1.5 rounded-full" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-coral-500/100' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-green-500'"></span>
            </div>
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-blue-500 hover:underline"
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-paper-200 dark:bg-ink-700">
            <div class="h-full transition-all" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-coral-500/100' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-green-500'" :style="{ width: `${Math.max((slaPercent ?? 0) - 90, 0) * 10}%` }"></div>
          </div>
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.exceptions') }}:</span>
              <span class="font-bold text-coral-600 dark:text-coral-500">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 4: Request Duration -->
        <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-900" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-dust-400">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-black text-primary-fg">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-bold text-dust-400">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-dust-500">P95:</span>
              <span class="font-bold text-primary-fg">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-dust-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-dust-500">P90:</span>
              <span class="font-bold text-primary-fg">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-dust-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-dust-500">P50:</span>
              <span class="font-bold text-primary-fg">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-dust-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-dust-500">Avg:</span>
              <span class="font-bold text-primary-fg">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-dust-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-dust-500">Max:</span>
              <span class="font-bold text-primary-fg">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-dust-400">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-900" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-dust-400">TTFT</span>
        ...
        <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-900" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-dust-400">{{ t('admin.ops.requestErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-primary-fg">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-primary-fg">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-900" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-dust-400">{{ t('admin.ops.upstreamErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-dust-500">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-primary-fg">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-dust-500">429/529:</span>
              <span class="font-bold text-primary-fg">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-paper-100 pt-4 dark:border-ink-700">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-paper-50 p-3 dark:bg-ink-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-dust-400">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-dust-500 dark:text-dust-400">
        ...
        <div class="rounded-xl bg-paper-50 p-3 dark:bg-ink-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-dust-400">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-paper-50 p-3 dark:bg-ink-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-dust-400">{{ t('admin.ops.db') }}</div>
        ...
        <div class="rounded-xl bg-paper-50 p-3 dark:bg-ink-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-dust-400">Redis</div>
        ...
        <div class="rounded-xl bg-paper-50 p-3 dark:bg-ink-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-dust-400">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="rounded-xl bg-paper-50 p-3 dark:bg-ink-900">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-dust-400">{{ t('admin.ops.jobs') }}</div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-dust-500 dark:text-dust-400">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        <div
          ...
          :key="hb.job_name"
          class="rounded-xl border border-paper-100 bg-white p-4 dark:border-ink-700 dark:bg-ink-900"
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-primary-fg">{{ hb.job_name }}</div>
            <div class="flex items-center gap-3 text-xs text-dust-500 dark:text-dust-400">
              <span v-if="hb.last_duration_ms != null" class="font-mono">{{ hb.last_duration_ms }}ms</span>
              <span>{{ formatTimeShort(hb.updated_at) }}</span>
            </div>
          </div>

          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-dust-600 dark:text-dust-300 sm:grid-cols-2">
            <div>
    ...
    <BaseDialog :show="showCustomTimeRangeDialog" :title="t('admin.ops.timeRange.custom')" width="narrow" @close="handleCustomTimeRangeCancel">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-dust-700 dark:text-dust-300 mb-1">
            {{ t('admin.ops.customTimeRange.startTime') }}
          </label>
          <input
            v-model="customStartTimeInput"
            type="datetime-local"
            class="w-full rounded-lg border border-paper-300 bg-white px-3 py-2 text-sm text-dust-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-ink-700 dark:bg-ink-800 dark:text-pearl-50"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-dust-700 dark:text-dust-300 mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-paper-100 px-4 py-2 text-sm font-medium text-dust-700 hover:bg-paper-200 dark:bg-ink-700 dark:text-dust-300 dark:hover:bg-ink-600"
