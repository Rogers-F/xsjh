  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-3">
        ...
        <div class="text-sm font-medium text-dust-500 dark:text-dust-400">{{ t('admin.ops.errorDetail.loading') }}</div>
      </div>
    </div>

    <div v-else-if="!detail" class="py-10 text-center text-sm text-dust-500 dark:text-dust-400">
      {{ emptyText }}
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-xl bg-paper-50 p-4 dark:bg-ink-900">
          ...
          <div class="mt-1 text-sm font-medium text-primary-fg">
            <template v-if="hasModelMapping(detail)">
              ...
              <span class="font-mono text-gold-600 dark:text-gold-400">{{ detail.upstream_model }}</span>
            </template>
            <template v-else>
              {{ displayModel(detail) || '—' }}
            </template>
        ...
        <div class="rounded-xl bg-paper-50 p-4 dark:bg-ink-900">
          <div class="text-xs font-bold uppercase tracking-wider text-dust-400">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
      ...
      <div v-if="showUpstreamList" class="rounded-xl bg-paper-50 p-6 dark:bg-ink-900">
        ...
        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-dust-500 dark:text-dust-400">
          {{ t('common.noData') }}
        </div>
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-2">
                <div class="font-mono text-xs text-dust-500 dark:text-dust-400">
                  {{ ev.status_code ?? '—' }}
                </div>
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-md px-1.5 py-1 text-[10px] font-bold text-gold-700 hover:bg-gold-500/10 disabled:cursor-not-allowed disabled:opacity-60 dark:text-gold-200 dark:hover:bg-ink-700"
            ...
            <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-dust-600 dark:text-dust-300 sm:grid-cols-2">
              <div>
                <span class="text-dust-400">{{ t('admin.ops.errorDetail.upstreamEvent.status') }}:</span>
                <span class="ml-1 font-mono">{{ ev.status_code ?? '—' }}</span>
              </div>
              <div>
                <span class="text-dust-400">{{ t('admin.ops.errorDetail.upstreamEvent.requestId') }}:</span>
            ...
            <pre
              ...
              class="mt-3 max-h-[240px] overflow-auto rounded-xl border border-paper-200 bg-paper-50 p-3 text-xs text-dust-800 dark:border-ink-700 dark:bg-ink-900 dark:text-pearl-100"
