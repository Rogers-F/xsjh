...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-ink-900/5 dark:bg-ink-800 dark:ring-ink-700">
    ...
    <div v-if="loading" class="py-10 text-center text-sm text-dust-500 dark:text-dust-400">
      {{ t('admin.ops.alertRules.loading') }}
    </div>

    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-paper-200 p-8 text-center text-sm text-dust-500 dark:border-ink-700 dark:text-dust-400">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-xl border hairline">
          ...
          <tbody class="divide-y divide-paper-200 bg-white dark:divide-ink-700 dark:bg-ink-800">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-paper-50 dark:hover:bg-ink-700/50">
              <td class="px-4 py-3">
                ...
                <div v-if="row.updated_at" class="mt-1 text-[10px] text-dust-400">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-dust-700 dark:text-pearl-200">
                <span class="font-mono">{{ row.metric_type }}</span>
                <span class="mx-1 text-dust-400">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-dust-700 dark:text-pearl-200">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-dust-700 dark:text-pearl-200">
    ...
    >
      <div class="space-y-4">
        <div v-if="!editorValidation.valid" class="rounded-xl bg-coral-500/10 p-4 text-xs text-coral-700 dark:bg-coral-500/15 dark:text-coral-400">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-dust-500 dark:text-dust-400">
          ...
          <div class="md:col-span-2">
            <label class="input-label">
              ...
              <span v-if="isGroupMetricSelected" class="ml-1 text-coral-500">*</span>
            </label>
            <Select
            ...
            />
            <p class="mt-1 text-xs text-dust-500 dark:text-dust-400">
          ...
          <div class="flex items-center justify-between rounded-xl bg-paper-50 px-4 py-3 dark:bg-ink-800/50 md:col-span-2">
            <span class="text-xs font-bold text-dust-700 dark:text-pearl-200">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-paper-300 text-gold-600 focus:ring-gold-500" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-paper-50 px-4 py-3 dark:bg-ink-800/50 md:col-span-2">
            <span class="text-xs font-bold text-dust-700 dark:text-pearl-200">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-paper-300 text-gold-600 focus:ring-gold-500" />
          </div>
        </div>
      </div>
