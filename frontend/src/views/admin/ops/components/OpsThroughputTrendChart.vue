  ...
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-ink-900/5 dark:bg-ink-800 dark:ring-ink-700">
    <div class="mb-4 flex shrink-0 items-center justify-between">
      ...
      <div class="flex items-center gap-2 text-xs text-secondary-fg">
        ...
        <template v-if="!props.fullscreen">
          <button
            type="button"
            class="ml-2 inline-flex items-center rounded-lg border hairline bg-white px-2 py-1 text-[11px] font-semibold text-dust-600 hover:bg-paper-50 disabled:opacity-50 dark:bg-ink-900 dark:text-pearl-100 dark:hover:bg-ink-800"
          ...
          <button
            type="button"
            class="inline-flex items-center rounded-lg border hairline bg-white px-2 py-1 text-[11px] font-semibold text-dust-600 hover:bg-paper-50 disabled:opacity-50 dark:bg-ink-900 dark:text-pearl-100 dark:hover:bg-ink-800"
            :disabled="state !== 'ready'"
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border hairline bg-white px-3 py-1 text-[11px] font-semibold text-dust-700 hover:bg-paper-50 dark:bg-ink-900 dark:text-pearl-50 dark:hover:bg-ink-800"
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-dust-400 dark:text-pearl-400">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-dust-400 dark:text-pearl-400">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
    ...
    <div class="min-h-0 flex-1">
      ...
      <div v-else class="flex h-full items-center justify-center">
        <div v-if="state === 'loading'" class="animate-pulse text-sm text-dust-400">{{ t('common.loading') }}</div>
