...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-ink-900/5 dark:bg-ink-800 dark:ring-ink-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      ...
      <button
        class="flex items-center gap-1.5 rounded-lg bg-paper-100 px-3 py-1.5 text-xs font-bold text-dust-700 transition-colors hover:bg-paper-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-ink-700 dark:text-dust-300 dark:hover:bg-ink-600"
        :disabled="loading"
        @click="loadSettings"
      >
    ...
    <div v-if="!alertSettings" class="text-sm text-dust-500 dark:text-dust-400">
      ...
      <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-700/50">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <details class="col-span-1 md:col-span-2">
            ...
            <div class="mt-2 grid grid-cols-1 gap-3 rounded-lg bg-paper-100 p-3 dark:bg-ink-800 md:grid-cols-2">
              <div class="text-xs text-dust-500 dark:text-dust-400">
                {{ t('admin.ops.runtime.lockEnabled') }}:
                <span class="ml-1 font-mono text-dust-700 dark:text-dust-300">{{ alertSettings.distributed_lock.enabled }}</span>
              </div>
              <div class="text-xs text-dust-500 dark:text-dust-400">
                {{ t('admin.ops.runtime.lockKey') }}:
                <span class="ml-1 font-mono text-dust-700 dark:text-dust-300">{{ alertSettings.distributed_lock.key }}</span>
              </div>
              <div class="text-xs text-dust-500 dark:text-dust-400">
                {{ t('admin.ops.runtime.lockTTLSeconds') }}:
                <span class="ml-1 font-mono text-dust-700 dark:text-dust-300">{{ alertSettings.distributed_lock.ttl_seconds }}s</span>
              </div>
            </div>
          </details>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="t('admin.ops.runtime.alertTitle')" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.evalIntervalSeconds') }}</div>
        <input
        ...
        />
        <p class="mt-1 text-xs text-dust-500">{{ t('admin.ops.runtime.evalIntervalHint') }}</p>
      </div>
      ...
      <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-700/50">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.slaMinPercent') }}</div>
            <input
              ...
              placeholder="99.5"
            />
            <p class="mt-1 text-xs text-dust-500 dark:text-dust-400">{{ t('admin.ops.runtime.slaMinPercentHint') }}</p>
          </div>



          <div>
            <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.ttftP99MaxMs') }}</div>
            <input
              ...
              placeholder="500"
            />
            <p class="mt-1 text-xs text-dust-500 dark:text-dust-400">{{ t('admin.ops.runtime.ttftP99MaxMsHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.requestErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-dust-500 dark:text-dust-400">{{ t('admin.ops.runtime.requestErrorRateMaxPercentHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-dust-500 dark:text-dust-400">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-2xl bg-paper-50 p-4 dark:bg-ink-700/50">
        ...
        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.silencing.globalUntil') }}</div>
            <input
            ...
            />
            <p class="mt-1 text-xs text-dust-500 dark:text-dust-400">{{ t('admin.ops.runtime.silencing.untilHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.silencing.reason') }}</div>
            <input
          ...
          <div class="rounded-xl border border-paper-200 bg-white p-4 dark:border-ink-700 dark:bg-ink-800">
            ...
            <div v-if="!draftAlert.silencing.entries?.length" class="mt-3 rounded-lg bg-paper-50 p-3 text-xs text-dust-500 dark:bg-ink-900 dark:text-dust-400">
            ...
            <div v-else class="mt-4 space-y-4">
              ...
              >
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.silencing.entries.ruleId') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.silencing.entries.severities') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.silencing.entries.until') }}</div>
                    <input
              ...
              placeholder="2026-01-05T00:00:00Z"
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.runtime.silencing.entries.reason') }}</div>
                    <input
      ...
      <details class="rounded-lg border border-paper-200 bg-paper-50 p-3 dark:border-ink-700 dark:bg-ink-800">
        <summary class="cursor-pointer text-xs font-medium text-dust-600 dark:text-dust-400">{{ t('admin.ops.runtime.advancedSettingsSummary') }}</summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="inline-flex items-center gap-2 text-xs text-dust-700 dark:text-dust-300">
              <input v-model="draftAlert.distributed_lock.enabled" type="checkbox" class="h-4 w-4 rounded border-paper-300" />
              <span>{{ t('admin.ops.runtime.lockEnabled') }}</span>
            </label>
          </div>
          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-dust-500">{{ t('admin.ops.runtime.lockKey') }}</div>
            <input v-model="draftAlert.distributed_lock.key" type="text" class="input text-xs font-mono" />
            <p v-if="draftAlert.distributed_lock.enabled" class="mt-1 text-[11px] text-dust-500 dark:text-dust-400">
              {{ t('admin.ops.runtime.validation.lockKeyHint', { prefix: 'ops:' }) }}
            </p>
          </div>
          <div>
            <div class="mb-1 text-xs font-medium text-dust-500">{{ t('admin.ops.runtime.lockTTLSeconds') }}</div>
