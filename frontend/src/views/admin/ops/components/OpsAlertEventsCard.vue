...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p0' || s === 'critical') return 'bg-coral-500/15 text-coral-700 dark:bg-coral-500/15 dark:text-coral-400'
  ...
  if (s === 'p3') return 'bg-paper-100 text-dust-700 dark:bg-ink-700 dark:text-dust-300'
  return 'bg-paper-100 text-dust-700 dark:bg-ink-700 dark:text-dust-300'
}

function statusBadgeClass(status: string | undefined): string {
  const s = String(status || '').trim().toLowerCase()
  if (s === 'firing') return 'bg-coral-500/10 text-coral-700 ring-coral-600/20 dark:bg-coral-500/15 dark:text-coral-400 dark:ring-coral-500/30'
  ...
  return 'bg-paper-50 text-dust-700 ring-dust-600/20 dark:bg-ink-900/30 dark:text-dust-300 dark:ring-dust-500/30'
}
...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-ink-900/5 dark:bg-ink-800 dark:ring-ink-700">
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-dust-500 dark:text-dust-400">
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-paper-200 p-8 text-center text-sm text-dust-500 dark:border-ink-700 dark:text-dust-400">
      {{ t('admin.ops.alertEvents.empty') }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border hairline">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <table class="min-w-full divide-y divide-paper-200 dark:divide-ink-700">
          <thead class="sticky top-0 z-10 bg-paper-50 dark:bg-ink-900">
            <tr>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-dust-500 dark:text-dust-400">
              ...
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-dust-500 dark:text-dust-400">
          ...
          <tbody class="divide-y divide-paper-200 bg-white dark:divide-ink-700 dark:bg-ink-800">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-paper-50 dark:hover:bg-ink-700/50"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 text-xs text-dust-600 dark:text-dust-300">
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-dust-700 dark:text-pearl-200">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-dust-500 dark:text-dust-400">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  ...
                  <Icon
                    ...
                    name="ban"
                    size="sm"
                    class="text-dust-400 dark:text-dust-500"
                  />
                  <span class="text-[11px] font-bold text-dust-600 dark:text-dust-300">
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-dust-500 dark:text-dust-400">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-dust-400">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-dust-500 dark:text-dust-400">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-dust-500 dark:text-dust-400">
      ...
      <div v-else class="space-y-5">
        <div class="rounded-xl bg-paper-50 p-4 dark:bg-ink-900">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-white px-2 py-1 ring-1 ring-paper-200 dark:bg-ink-800 dark:ring-ink-700">
                <span class="text-[11px] font-bold text-dust-600 dark:text-dust-300">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
        ...
        <div class="rounded-xl border border-paper-200 bg-white p-4 dark:border-ink-700 dark:bg-ink-800">
          ...
          <div v-if="historyLoading" class="py-6 text-center text-xs text-dust-500 dark:text-dust-400">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-dust-500 dark:text-dust-400">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border hairline">
            <table class="min-w-full divide-y divide-paper-100 dark:divide-ink-700">
              <thead class="bg-paper-50 dark:bg-ink-900">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-dust-500 dark:text-dust-400">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-dust-500 dark:text-dust-400">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-dust-500 dark:text-dust-400">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-paper-100 dark:divide-ink-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-paper-50 dark:hover:bg-ink-700/50">
                  <td class="px-3 py-2 text-xs text-dust-600 dark:text-dust-300">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
                  ...
                  <td class="px-3 py-2 text-xs text-dust-600 dark:text-dust-300">
