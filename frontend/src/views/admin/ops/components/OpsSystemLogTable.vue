...
const levelBadgeClass = (level: string) => {
  ...
  if (v === 'error' || v === 'fatal') return 'bg-coral-500/15 text-coral-700 dark:bg-coral-500/15 dark:text-coral-400'
  ...
  <section class="rounded-2xl border border-paper-200 bg-white p-4 shadow-sm dark:border-ink-700 dark:bg-ink-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-paper-100 px-2 py-1 text-dust-700 dark:bg-ink-700 dark:text-pearl-200">队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-paper-100 px-2 py-1 text-dust-700 dark:bg-ink-700 dark:text-pearl-200">写入 {{ health.written_count }}</span>
        ...
        <span class="rounded-md bg-coral-500/15 px-2 py-1 text-coral-700 dark:bg-coral-500/15 dark:text-coral-400">失败 {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-paper-200 bg-paper-50 p-3 dark:border-ink-700 dark:bg-ink-800/70">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-dust-700 dark:text-pearl-200">运行时日志配置（实时生效）</div>
        <span v-if="runtimeLoading" class="text-xs text-dust-500">加载中...</span>
      </div>
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
        <label class="text-xs text-dust-600 dark:text-dust-300">
          级别
        ...
        <div class="md:col-span-2 xl:col-span-6">
          <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
              <label class="inline-flex items-center gap-2 text-xs text-dust-600 dark:text-dust-300">
      ...
      <p v-if="health.last_error" class="mt-2 text-xs text-coral-600 dark:text-coral-500">最近写入错误：{{ health.last_error }}</p>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border hairline">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-dust-500">加载中...</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-dust-500">暂无系统日志</div>
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-paper-200 dark:divide-ink-700">
          <thead class="bg-paper-50 dark:bg-ink-900">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-dust-500">时间</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-dust-500">级别</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-dust-500">日志详细信息</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-paper-100 dark:divide-ink-800">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              <td class="px-3 py-2 text-xs text-dust-700 dark:text-dust-300">{{ formatTime(row.created_at) }}</td>
              <td class="px-3 py-2 text-xs">
                <span class="inline-flex rounded-full px-2 py-0.5 font-semibold" :class="levelBadgeClass(row.level)">
                  {{ row.level }}
                </span>
              </td>
              <td class="px-3 py-2 text-xs text-dust-700 dark:text-dust-300 whitespace-normal break-all">
