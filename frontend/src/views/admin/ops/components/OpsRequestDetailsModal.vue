...
const kindBadgeClass = (kind: string) => {
  if (kind === 'error') return 'bg-coral-500/15 text-coral-700 dark:bg-coral-500/15 dark:text-coral-400'
      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm font-medium text-dust-500 dark:text-dust-400">{{ t('common.loading') }}</span>
          </div>
        </div>

        <!-- Table -->
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-paper-200 p-10 text-center dark:border-ink-700">
            <div class="text-sm font-medium text-dust-600 dark:text-dust-300">{{ t('admin.ops.requestDetails.empty') }}</div>
            <div class="mt-1 text-xs text-dust-400">{{ t('admin.ops.requestDetails.emptyHint') }}</div>
          </div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border hairline">
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <tbody class="divide-y divide-paper-200 bg-white dark:divide-ink-700 dark:bg-ink-800">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-paper-50 dark:hover:bg-ink-700/50">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-xs font-medium text-dust-700 dark:text-pearl-200">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-dust-600 dark:text-dust-300" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-dust-600 dark:text-dust-300">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-dust-700 dark:text-pearl-200" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button
                        class="rounded-md bg-paper-100 px-2 py-1 text-[10px] font-bold text-dust-600 hover:bg-paper-200 dark:bg-ink-700 dark:text-dust-300 dark:hover:bg-ink-600"
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-right">
                    <button
                      ...
                      class="rounded-lg bg-coral-500/10 px-3 py-1.5 text-xs font-bold text-coral-600 hover:bg-coral-500/15 dark:bg-coral-500/15 dark:text-coral-400 dark:hover:bg-coral-500/15"
