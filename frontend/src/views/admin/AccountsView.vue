<template>
  <AppLayout>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-lg border border-paper-200 bg-paper-0 shadow-lg dark:border-ink-700 dark:bg-ink-800"
                >
                  <div class="p-2">
                    <button
                      @click="setAutoRefreshEnabled(!autoRefreshEnabled)"
                      class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-ink-700 hover:bg-paper-100 dark:text-paper-200 dark:hover:bg-white/[0.04]"
                    >
                      <span>{{ t('admin.accounts.enableAutoRefresh') }}</span>
                      <Icon v-if="autoRefreshEnabled" name="check" size="sm" class="text-gold-500" />
                    </button>
                    <div class="my-1 border-t border-paper-100 dark:border-ink-700"></div>
                    <button
                    ...
                    >
                      ...
                      <Icon v-if="autoRefreshIntervalSeconds === sec" name="check" size="sm" class="text-gold-500" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div class="relative" ref="columnDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-lg border border-paper-200 bg-paper-0 shadow-lg dark:border-ink-700 dark:bg-ink-800"
                >
                  <div class="max-h-80 overflow-y-auto p-2">
                    ...
                    >
                      <span>{{ col.label }}</span>
                      <Icon v-if="isColumnVisible(col.key)" name="check" size="sm" class="text-gold-500" />
                    </button>
                  </div>
                </div>
              </div>
      ...
      <template #table>
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="h-4 w-4 cursor-pointer rounded border-paper-300 text-gold-600 focus:ring-gold-500"
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="selIds.includes(row.id)" @change="toggleSel(row.id)" class="rounded border-paper-300 text-gold-600 focus:ring-gold-500" />
          </template>
          <template #cell-name="{ row, value }">
            <div class="flex flex-col">
              <span class="font-medium text-ink-900 dark:text-white">{{ value }}</span>
              <span
                v-if="row.extra?.email_address"
                class="text-xs text-dust-500 dark:text-pearl-400 truncate max-w-[200px]"
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-ink-600 dark:text-paper-100">{{ value }}</span>
            <span v-else class="text-sm text-dust-400 dark:text-pearl-500">-</span>
          </template>
          ...
          <template #cell-schedulable="{ row }">
            <button @click="handleToggleSchedulable(row)" :disabled="togglingSchedulable === row.id" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-gold-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:focus:ring-offset-ink-800" :class="[row.schedulable ? 'bg-gold-500 hover:bg-gold-600' : 'bg-paper-200 hover:bg-paper-300 dark:bg-ink-600 dark:hover:bg-ink-500']" :title="row.schedulable ? t('admin.accounts.schedulableEnabled') : t('admin.accounts.schedulableDisabled')">
          ...
          <template #cell-total_stats="{ row }">
            <AccountTotalStatsCell :account="row" />
          </template>
          <template #cell-created_at="{ value }">
            <span class="text-sm text-dust-500 dark:text-pearl-400">{{ formatRelativeTime(value) }}</span>
          </template>
          ...
          <template #cell-proxy="{ row }">
            <div v-if="row.proxy" class="flex items-center gap-2">
              <span class="text-sm text-ink-700 dark:text-paper-100">{{ row.proxy.name }}</span>
              <span v-if="row.proxy.country_code" class="text-xs text-dust-500 dark:text-pearl-400">
          ...
          <template #cell-rate_multiplier="{ row }">
            <span class="text-sm font-mono text-ink-700 dark:text-paper-100">
              {{ (row.rate_multiplier ?? 1).toFixed(2) }}x
            </span>
          </template>
          <template #cell-priority="{ value }">
            <span class="text-sm text-ink-700 dark:text-paper-100">{{ value }}</span>
          </template>
          ...
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-dust-500 dark:text-pearl-400">{{ formatExpiresAt(value) }}</span>
              <div v-if="isExpired(value) || (row.auto_pause_on_expired && value)" class="flex items-center gap-1">
                ...
                <span
                  ...
                  class="inline-flex items-center rounded-md bg-mint-500/15 px-2 py-0.5 text-xs font-medium text-mint-700 dark:bg-mint-500/15 dark:text-mint-400"
                >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors hover:bg-paper-100 hover:text-gold-600 dark:hover:bg-white/[0.04] dark:hover:text-gold-400">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors hover:bg-coral-500/15 hover:text-coral-600 dark:hover:bg-coral-500/15 dark:hover:text-coral-400">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                <span class="text-xs">{{ t('common.delete') }}</span>
              </button>
              <button @click="openMenu(row, $event)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors hover:bg-paper-100 hover:text-ink-900 dark:hover:bg-white/[0.04] dark:hover:text-white">
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-ink-700 dark:text-paper-100">
        <input type="checkbox" class="h-4 w-4 rounded border-paper-300 text-gold-600 focus:ring-gold-500" v-model="includeProxyOnExport" />
...
import AccountTotalStatsCell from '@/components/account/AccountTotalStatsCell.vue'
...
function getAntigravityTierClass(row: any): string {
  const tier = getAntigravityTierFromRow(row)
  switch (tier) {
    case 'free-tier': return 'bg-paper-100 text-ink-600 dark:bg-ink-700 dark:text-paper-100'
...
const allColumns = computed(() => {
  const c = [
    ...
    { key: 'total_stats', label: t('admin.accounts.columns.totalStats'), sortable: false }
  ]
