  ...
  <AppLayout>
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            <div class="relative w-full md:w-64">
              <Icon
                name="search"
                size="md"
                class="absolute left-3 top-1/2 -translate-y-1/2 text-dust-400"
              />
              <input
                v-model="searchQuery"
                type="text"
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 w-48 rounded-lg border border-paper-200 bg-white py-1 shadow-lg dark:border-ink-700 dark:bg-ink-800"
                >
                  <!-- Built-in filters -->
                  <button
                    ...
                    class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-dust-700 hover:bg-paper-100 dark:text-pearl-100 dark:hover:bg-white/[0.04]"
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      class="text-gold-500 dark:text-gold-300"
                      :stroke-width="2"
                    />
                  </button>
              ...
              <div class="relative" ref="columnDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-paper-200 bg-white py-1 shadow-lg dark:border-ink-700 dark:bg-ink-800"
                >
                  <button
        ...
        <DataTable :columns="columns" :data="users" :loading="loading" :actions-count="7">
          <template #cell-email="{ value }">
            <div class="flex items-center gap-2">
              <div
                class="flex h-8 w-8 items-center justify-center rounded-full bg-gold-500/15 dark:bg-gold-300/15"
              >
          ...
          <template #cell-username="{ value }">
            <span class="text-sm text-dust-700 dark:text-pearl-100">{{ value || '-' }}</span>
          </template>
          ...
          <template #cell-notes="{ value }">
            <div class="max-w-xs">
              <span
                ...
                class="block truncate text-sm text-dust-600 dark:text-pearl-300"
              >
          ...
          >
            <div class="max-w-xs">
              <span
                class="block truncate text-sm text-dust-700 dark:text-pearl-100"
            ...
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                ...
                <div
                  ...
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-ink-900 px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/ex:opacity-100 dark:bg-ink-700"
                >
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-ink-900 dark:border-b-ink-700"></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-paper-200 bg-white py-1 text-xs shadow-xl dark:border-ink-700 dark:bg-ink-700"
                >
                  <div class="hairline border-b px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-dust-400 dark:text-pearl-300">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-dust-700 transition-colors hover:bg-gold-500/10 hover:text-gold-600 dark:text-pearl-100 dark:hover:bg-gold-300/15 dark:hover:text-gold-300"
              ...
              >
                <Icon name="globe" size="xs" class="h-3.5 w-3.5 text-dust-400 dark:text-pearl-400" />
                <span class="font-medium text-dust-600 dark:text-pearl-200">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-dust-400 dark:text-pearl-400">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-ink-900 px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100 dark:bg-ink-700">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-ink-900 dark:border-b-ink-700"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
              ...
              <span
                ...
                class="text-xs text-dust-400 dark:text-pearl-400"
              >-</span>
            </div>
            <span v-else class="text-xs text-dust-400 dark:text-pearl-400">-</span>
          </template>
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-paper-50 px-2 py-1 text-xs text-dust-400 dark:bg-ink-700/50 dark:text-pearl-400"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                <button
                  class="font-medium text-dust-900 underline decoration-dashed decoration-paper-300 underline-offset-4 transition-colors hover:text-gold-600 dark:text-pearl-50 dark:decoration-ink-700 dark:hover:text-gold-300"
                  @click="handleBalanceHistory(row)"
                >
                  ${{ value.toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-ink-900 px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100 dark:bg-ink-700">
                  {{ t('admin.users.balanceHistoryTip') }}
                  <div class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-ink-900 dark:border-t-ink-700"></div>
                </div>
              </div>
              <button
                @click.stop="handleDeposit(row)"
                class="rounded px-2 py-0.5 text-xs font-medium text-mint-600 transition-colors hover:bg-mint-500/10 dark:text-mint-500 dark:hover:bg-mint-500/15"
            ...
            <div class="flex items-center gap-1.5">
              <span
                :class="[
                  ...
                  value === 'active' ? 'bg-green-500' : 'bg-coral-500'
                ]"
              ></span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  'flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors',
                  row.status === 'active'
              ...
              <button
                ...
                class="action-menu-trigger flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors hover:bg-paper-100 hover:text-dust-900 dark:hover:bg-white/[0.04] dark:hover:text-pearl-50"
                :class="{ 'bg-paper-100 text-dust-900 dark:bg-white/[0.04] dark:text-pearl-50': activeMenuId === row.id }"
              >
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content fixed z-[9999] w-48 overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black/5 dark:bg-ink-800 dark:ring-white/10"
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-dust-700 hover:bg-paper-100 dark:text-pearl-100 dark:hover:bg-white/[0.04]"
              >
                <Icon name="key" size="sm" class="text-dust-400" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-dust-700 hover:bg-paper-100 dark:text-pearl-100 dark:hover:bg-white/[0.04]"
              >
                <Icon name="users" size="sm" class="text-dust-400" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-dust-700 hover:bg-paper-100 dark:text-pearl-100 dark:hover:bg-white/[0.04]"
              >
                <span class="flex h-4 w-4 items-center justify-center text-xs font-semibold text-dust-400">%</span>
                {{ t('admin.users.commissionRate.action') }}
              </button>

              <div class="my-1 hairline border-t"></div>
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-dust-700 hover:bg-paper-100 dark:text-pearl-100 dark:hover:bg-white/[0.04]"
              >
                <Icon name="plus" size="sm" class="text-mint-500" :stroke-width="2" />
                {{ t('admin.users.deposit') }}
              </button>
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-dust-700 hover:bg-paper-100 dark:text-pearl-100 dark:hover:bg-white/[0.04]"
              >
                <Icon name="dollar" size="sm" class="text-dust-400" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-coral-600 hover:bg-coral-500/10 dark:text-coral-500 dark:hover:bg-coral-500/15"
              >
