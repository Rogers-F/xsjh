    ...
    <div class="space-y-6">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-sm text-dust-500 dark:text-pearl-400">
              ...
              <button type="button" class="text-gold-600 underline hover:text-gold-700 dark:text-gold-400 dark:hover:text-gold-300" @click="showR2Guide = true">Cloudflare R2</button>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.s3.endpoint') }}</label>
            <input v-model="s3Form.endpoint" class="input w-full" placeholder="https://<account_id>.r2.cloudflarestorage.com" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.s3.region') }}</label>
            <input v-model="s3Form.region" class="input w-full" placeholder="auto" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.s3.bucket') }}</label>
            <input v-model="s3Form.bucket" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.s3.prefix') }}</label>
            <input v-model="s3Form.prefix" class="input w-full" placeholder="backups/" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.s3.accessKeyId') }}</label>
            <input v-model="s3Form.access_key_id" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.s3.secretAccessKey') }}</label>
      ...
      <div class="card p-6">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.schedule.cronExpr') }}</label>
            <input v-model="scheduleForm.cron_expr" class="input w-full" placeholder="0 2 * * *" />
            <p class="mt-1 text-xs text-dust-500 dark:text-pearl-400">{{ t('admin.backup.schedule.cronHint') }}</p>
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.schedule.retainDays') }}</label>
            <input v-model.number="scheduleForm.retain_days" type="number" min="0" class="input w-full" />
            <p class="mt-1 text-xs text-dust-500 dark:text-pearl-400">{{ t('admin.backup.schedule.retainDaysHint') }}</p>
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-pearl-400">{{ t('admin.backup.schedule.retainCount') }}</label>
            <input v-model.number="scheduleForm.retain_count" type="number" min="0" class="input w-full" />
            <p class="mt-1 text-xs text-dust-500 dark:text-pearl-400">{{ t('admin.backup.schedule.retainCountHint') }}</p>
          </div>
        </div>
        <div class="mt-4">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <div class="flex items-center gap-1">
              <label class="text-xs text-ink-600 dark:text-pearl-400">{{ t('admin.backup.operations.expireDays') }}</label>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[800px] text-sm">
            ...
            <tbody>
              <tr v-for="record in backups" :key="record.id" class="border-b border-paper-100 align-top dark:border-ink-800">
              ...
              <tr v-if="backups.length === 0">
                <td colspan="8" class="py-6 text-center text-sm text-dust-500 dark:text-pearl-400">
    ...
    <teleport to="body">
      <transition name="modal">
        <div v-if="showR2Guide" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showR2Guide = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-paper-0 p-6 shadow-2xl dark:bg-ink-800">
            <button type="button" class="absolute right-4 top-4 text-dust-400 hover:text-ink-600 dark:hover:text-paper-200" @click="showR2Guide = false">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <h2 class="mb-4 text-lg font-bold text-ink-900 dark:text-white">{{ t('admin.backup.r2Guide.title') }}</h2>
            <p class="mb-4 text-sm text-dust-500 dark:text-pearl-400">{{ t('admin.backup.r2Guide.intro') }}</p>

            <!-- Step 1 -->
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-ink-900 dark:text-white">
              ...
              <ol class="ml-8 list-decimal space-y-1 text-sm text-ink-600 dark:text-paper-100">
            ...
            <div class="mb-5">
              ...
              <p class="ml-8 text-sm text-ink-600 dark:text-paper-100">{{ t('admin.backup.r2Guide.step3.desc') }}</p>
              <code class="ml-8 mt-1 block rounded bg-paper-100 px-3 py-2 text-xs text-ink-800 dark:bg-ink-700 dark:text-paper-200">https://&lt;{{ t('admin.backup.r2Guide.step3.accountId') }}&gt;.r2.cloudflarestorage.com</code>
            </div>

            <!-- Step 4: Fill form -->
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-ink-900 dark:text-white">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-gold-500/15 text-xs font-bold text-gold-700 dark:bg-gold-300/15 dark:text-gold-300">4</span>
                {{ t('admin.backup.r2Guide.step4.title') }}
              </h3>
              <div class="ml-8 overflow-hidden rounded-lg border border-paper-200 dark:border-ink-600">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in r2ConfigRows" :key="i" class="border-b border-paper-100 dark:border-ink-700 last:border-0">
                      <td class="whitespace-nowrap bg-paper-50 px-3 py-2 font-medium text-ink-700 dark:bg-ink-700 dark:text-paper-100">{{ row.field }}</td>
                      <td class="px-3 py-2 text-ink-600 dark:text-pearl-400"><code class="text-xs">{{ row.value }}</code></td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
function statusClass(status: string): string {
  switch (status) {
    case 'completed':
      return 'bg-mint-500/15 text-mint-700 dark:bg-mint-500/15 dark:text-mint-400'
    case 'running':
      return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300'
    case 'failed':
      return 'bg-coral-500/15 text-coral-700 dark:bg-coral-500/15 dark:text-coral-400'
    default:
      return 'bg-paper-100 text-ink-700 dark:bg-ink-800 dark:text-paper-100'
  }
}
