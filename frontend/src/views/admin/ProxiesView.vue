<template>
  <AppLayout>
        ...
        <DataTable :columns="columns" :data="proxies" :loading="loading">
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded p-0.5 text-dust-400 hover:text-gold-600 dark:hover:text-gold-300"
                ...
                <div
                  ...
                  class="absolute left-0 top-full z-50 mt-1 w-auto min-w-[180px] rounded-lg border border-paper-200 bg-white py-1 shadow-lg dark:border-ink-700 dark:bg-ink-700"
                >
                  <button
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              ...
              <button
                v-if="row.password"
                type="button"
                class="ml-1 rounded p-0.5 text-dust-400 hover:text-dust-700 dark:hover:text-pearl-100"
          ...
          <template #cell-location="{ row }">
            <div class="flex items-center gap-2">
              ...
              />
              <span v-if="formatLocation(row)" class="text-sm text-dust-700 dark:text-pearl-100">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="text-sm text-dust-400">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="inline-flex items-center rounded bg-paper-100 px-2 py-0.5 text-xs font-medium text-gold-700 hover:bg-paper-200 dark:bg-ink-700 dark:text-gold-300 dark:hover:bg-white/[0.04]"
            ...
            <span
              v-else
              class="inline-flex items-center rounded bg-paper-100 px-2 py-0.5 text-xs font-medium text-dust-800 dark:bg-ink-700 dark:text-pearl-100"
            >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors hover:bg-mint-500/10 hover:text-mint-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-mint-500/15 dark:hover:text-mint-500"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-dust-500 transition-colors hover:bg-coral-500/10 hover:text-coral-600 dark:hover:bg-coral-500/15 dark:hover:text-coral-500"
              >
    ...
    >
      <!-- Tab Switch -->
      <div class="mb-6 flex hairline border-b">
        <button
          type="button"
          ...
          :class="[
            ...
            createMode === 'standard'
              ...
              : 'border-transparent text-dust-500 hover:text-dust-700 dark:text-pearl-300 dark:hover:text-pearl-100'
          ]"
        >
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-dust-400 hover:text-dust-700 dark:hover:text-pearl-100"
      ...
      <div v-else class="space-y-5">
        ...
        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="rounded-lg bg-paper-50 p-4 dark:bg-ink-700">
            <div class="flex items-center gap-4 text-sm">
              <div class="flex items-center gap-1.5">
              <Icon name="checkCircle" size="sm" :stroke-width="2" class="text-gold-500 dark:text-gold-300" />
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        <div class="rounded-lg border border-paper-200 bg-paper-50 p-4 dark:border-ink-700 dark:bg-ink-700">
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-dust-600 dark:text-pearl-100">
        ...
        <div class="max-h-80 overflow-auto rounded-lg border border-paper-200 dark:border-ink-700">
          <table class="min-w-full divide-y divide-paper-200 text-sm dark:divide-ink-700">
            <thead class="bg-paper-50 text-xs uppercase text-dust-500 dark:bg-ink-800 dark:text-pearl-300">
              <tr>
            ...
            <tbody class="divide-y divide-paper-200 bg-white dark:divide-ink-700 dark:bg-ink-900">
              <tr v-for="item in qualityReport.items" :key="item.target">
                ...
                <td class="px-3 py-2 text-dust-600 dark:text-pearl-100">{{ item.http_status ?? '-' }}</td>
                <td class="px-3 py-2 text-dust-600 dark:text-pearl-100">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_ms}ms` : '-' }}
                </td>
                <td class="px-3 py-2 text-dust-600 dark:text-pearl-100">
                  <span>{{ item.message || '-' }}</span>
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-dust-400">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div v-if="accountsLoading" class="flex items-center justify-center py-8 text-sm text-dust-500">
        <Icon name="refresh" size="md" class="mr-2 animate-spin" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="py-6 text-center text-sm text-dust-500">
        ...
        <table class="min-w-full divide-y divide-paper-200 text-sm dark:divide-ink-700">
          ...
          <tbody class="divide-y divide-paper-200 bg-white dark:divide-ink-700 dark:bg-ink-900">
            <tr v-for="account in proxyAccounts" :key="account.id">
              ...
              <td class="px-4 py-2 text-dust-600 dark:text-pearl-100">
                {{ account.notes || '-' }}
              </td>
            </tr>
          </tbody>
