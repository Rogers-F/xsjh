...
// QueryReferee chains the current query on the "referee" edge.
func (_q *ReferralRewardQuery) QueryReferee() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *ReferralRewardQuery) Clone() *ReferralRewardQuery {
	...
	return &ReferralRewardQuery{
		...
		withReferee:  _q.withReferee.Clone(),
...
// WithReferee tells the query-builder to eager-load the nodes that are connected to
// the "referee" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ReferralRewardQuery) WithReferee(opts ...func(*UserQuery)) *ReferralRewardQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withReferee = query
	return _q
}
...
//		GroupBy(referralreward.FieldReferrerID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ReferralReward.Query().
//		Select(referralreward.FieldReferrerID).
//		Scan(ctx, &v)
...
func (_q *ReferralRewardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ReferralReward, error) {
	...
	if query := _q.withReferee; query != nil {
		if err := _q.loadReferee(ctx, query, nodes, nil,
			func(n *ReferralReward, e *User) { n.Edges.Referee = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ReferralRewardQuery) loadReferee(ctx context.Context, query *UserQuery, nodes []*ReferralReward, init func(*ReferralReward), assign func(*ReferralReward, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*ReferralReward)
	for i := range nodes {
		fk := nodes[i].RefereeID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "referee_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ReferralRewardQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withReferee != nil {
			_spec.Node.AddColumnOnce(referralreward.FieldRefereeID)
		}
	}
