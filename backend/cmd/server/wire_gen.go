...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	referralService := service.NewReferralService(referralRepository, userRepository, settingService)
	authService := service.ProvideAuthServiceWithReferral(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, referralService)
	newAPITokenRepository := repository.NewNewAPITokenRepository(client, db)
	newAPIClient := repository.ProvideNewAPIClient(configConfig)
	newAPIProvisionService := service.NewNewAPIProvisionService(newAPITokenRepository, newAPIClient)
	userService := service.NewUserService(userRepository, apiKeyAuthCacheInvalidator, billingCache, newAPIProvisionService)
	redeemCache := repository.NewRedeemCache(redisClient)
	redeemService := service.ProvideRedeemServiceWithReferral(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, referralService)
	...
	paygOrderRepository := repository.NewPaygOrderRepository(client, db)
	paygService := service.NewPaygService(paygOrderRepository, userRepository, settingService, referralService, billingCache, apiKeyAuthCacheInvalidator, client)
	paygHandler := admin.NewPaygHandler(paygService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, paygHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, adminAPIKeyHandler, scheduledTestHandler)
	...
	referralHandler := handler.NewReferralHandler(referralService, settingService)
	handlerPaygHandler := handler.NewPaygHandler(paygService)
	handlerSettingHandler := handler.ProvideSettingHandler(settingService, buildInfo)
	totpHandler := handler.NewTotpHandler(totpService)
	conversationRepository := repository.NewConversationRepository(client)
	messageRepository := repository.NewMessageRepository(client)
	conversationService := service.NewConversationService(client, conversationRepository, messageRepository)
	conversationHandler := handler.NewConversationHandler(conversationService)
	newAPIRelayHandler := handler.NewNewAPIRelayHandler(configConfig, newAPIProvisionService, subscriptionService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, soraGatewayHandler, soraClientHandler, referralHandler, handlerPaygHandler, handlerSettingHandler, totpHandler, conversationHandler, newAPIRelayHandler, idempotencyCoordinator, idempotencyCleanupService)
