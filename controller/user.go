...
func Logout(c *gin.Context) {
	session := sessions.Default(c)
	// CSRF guard: when a session exists, require the New-Api-User header to match
	// the session id. A cross-site request (form / img / top-level navigation)
	// cannot set a custom header, so this blocks logout-CSRF under SameSite=Lax.
	// There is intentionally NO session_version / enabled check, so a stale or
	// disabled session can still clear its own cookie. No session => idempotent.
	if sid, ok := session.Get("id").(int); ok {
		if c.Request.Header.Get("New-Api-User") != strconv.Itoa(sid) {
