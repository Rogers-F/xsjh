...
func RequestEpay(c *gin.Context) {
	// Same compliance gate as redemption/subscription purchases — creating
	// payment orders is forbidden until the admin confirms the terms. Replied in
	// this endpoint's native {message,data} envelope.
	if !operation_setting.IsPaymentComplianceConfirmed() {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.TranslateMessage(c, i18n.MsgPaymentComplianceRequired)})
		return
	}
	var req EpayRequest
...
func RequestAmount(c *gin.Context) {
	// Compliance gate mirrors RequestEpay: no payable previews before the admin
	// confirms the terms.
	if !operation_setting.IsPaymentComplianceConfirmed() {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.TranslateMessage(c, i18n.MsgPaymentComplianceRequired)})
		return
