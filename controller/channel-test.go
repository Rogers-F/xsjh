...
func testChannel(channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	...
	// OAuth (Claude subscription) channels must apply the same mimic body transform here
	// as the live relay path — otherwise the test request reaches Anthropic without the
	// Claude Code banner / metadata / model normalization an OAuth token requires.
	if info.IsOAuth {
		jsonData = relay.ApplyClaudeOAuthTransform(info, jsonData)
	}
