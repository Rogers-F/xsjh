...
func clearChannelInfo(channel *model.Channel) {
	...
	// Defense-in-depth: never let an OAuth subscription blob (which contains the refresh
	// token) ride out through a channel object. redactOAuthChannelKey owns the single
	// predicate and returns the key unchanged for non-OAuth channels.
	if channel.Key != "" {
		channel.Key = redactOAuthChannelKey(channel)
	}
}

// isOAuthChannelKey reports whether a single key string is a Claude OAuth subscription
// blob. Used across the controller package to hard-disable control-plane probes (balance,
// model fetch, upstream sync) that would otherwise forward the blob as a credential.
func isOAuthChannelKey(key string) bool {
	return service.LooksLikeOAuthBlob(key)
}

// channelHasOAuthBlobKey reports whether ANY of a channel's keys is an OAuth credential
// blob. Control-plane probes pick a single key from a multi-key channel downstream, so
// checking only the concatenated channel.Key (one JSON parse) would miss a blob hidden
// among multi-key entries — scan each key.
func channelHasOAuthBlobKey(channel *model.Channel) bool {
	if channel == nil {
		return false
	}
	// Check the whole key FIRST: a single pretty-printed (multi-line) blob is one valid
	// JSON object, but GetKeys() would split it on newlines into fragments that each fail
	// the blob test. Then scan each multi-key entry for a blob hidden among them.
	if isOAuthChannelKey(strings.TrimSpace(channel.Key)) {
		return true
	}
	for _, k := range channel.GetKeys() {
		if isOAuthChannelKey(strings.TrimSpace(k)) {
			return true
		}
	}
	return false
...
// redactOAuthChannelKey returns a display-safe representation of a channel key. For a
// Claude OAuth subscription blob it strips the access/refresh tokens, returning only
// non-secret metadata (email, account uuid, expiry, last-4 of the access token); any
// other key is returned unchanged.
func redactOAuthChannelKey(ch *model.Channel) string {
	if ch == nil {
		return ""
	}
	if ch.Type != constant.ChannelTypeAnthropic || !service.LooksLikeOAuthBlob(ch.Key) {
		return ch.Key
	}
	k, err := service.ParseClaudeOAuthKey(ch.Key)
	if err != nil {
		return `{"auth_mode":"oauth","redacted":true}`
	}
	summary := k.RedactedSummary()
	summary["auth_mode"] = "oauth"
...
func GetChannelKey(c *gin.Context) {
	...
	// 返回渠道密钥（OAuth 订阅凭据脱敏，不回显 refresh/access token）
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "获取成功",
		"data": map[string]interface{}{
			"key": redactOAuthChannelKey(channel),
		},
	})
}
...
func FetchModels(c *gin.Context) {
	...
	// An OAuth subscription credential (Claude auth_mode=oauth) must never be sent on
	// the control plane: FetchModels has no channel context to authenticate it, and
	// forwarding the blob as a Bearer would leak the refresh token upstream. Check the
	// whole raw key AND each newline-separated entry (so a blob among multi-key paste, or
	// a pretty-printed blob, is also refused).
	rawKey := strings.TrimSpace(req.Key)
	rejectOAuth := isOAuthChannelKey(rawKey)
	...
	if rejectOAuth {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "OAuth 订阅凭据不支持在此探测模型列表，请在渠道编辑页手动维护模型",
		})
		return
	}
