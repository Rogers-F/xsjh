...
func fetchChannelUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	// An OAuth subscription blob must never be forwarded as a credential. The default
	// branch funnels through the GetResponseBody choke point, but the Ollama/Gemini
	// branches below return BEFORE it and forward a SINGLE selected key (Bearer /
	// x-goog-api-key) — scan every key here so a blob hidden among multi-key entries is
	// also refused (defense-in-depth, covers every branch).
	if channelHasOAuthBlobKey(channel) {
		return nil, fmt.Errorf("OAuth 订阅渠道不支持上游模型探测")
	}
