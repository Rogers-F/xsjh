...
// withSessionVersionDB points model.DB at an isolated in-memory SQLite that
// holds the one user the authenticated header-nav tests log in as, so
// authHelper's session_version lookup (session-cookie auth) reads a real row.
// Unique per-test DSN plus a single pooled connection keep the shared-cache
// memory DB alive and isolated; middleware tests run serially (no t.Parallel),
// so swapping the package-global DB pointer is safe.
func withSessionVersionDB(t *testing.T) {
	t.Helper()
	...
	require.NoError(t, db.Create(&model.User{
		Id:             1,
		Username:       "tester",
		Password:       "header-nav-test-password",
...
func performHeaderNavRequest(t *testing.T, handler gin.HandlerFunc, authenticated bool) *httptest.ResponseRecorder {
	...
	router.GET("/login", func(c *gin.Context) {
		...
		// Stamped at real login (controller/user.go setupLogin); authHelper
		// rejects sessions whose version no longer matches the user row.
